...
def bigquery_to_cobol(project_id: str, dataset_id: str, table_id: str, copybook_name: str = "TABLE-RECORD."):
    """
    Generates a COBOL copybook for a given BigQuery table.
    ...
    try:
        ...
        return f"Error: Could not retrieve table information. {e}"

    copybook_lines = [
        f"       01 {copybook_name.upper()}"
    ]
    
    # Helper function for indentation
    def get_indent(level):
        return " " * (level * 4) # 4 spaces per level for COBOL-like indentation
    ...
    def process_field(field, current_level):
        ...
        if mode == "REPEATED":
            ...
            cobol_lines.append(f"{get_indent(current_level)}   10 {field_name_cobol} OCCURS 999999 TIMES DEPENDING ON {field_name_cobol}-COUNT.")
            ...
            if field.fields:
                ...
                cobol_lines.append(f"{get_indent(current_level+1)}   {field_level} {field_name_cobol}-ITEM PIC X(256).") # Default
            ...
            cobol_lines.append(f"{get_indent(current_level)}   {field_level} {field_name_cobol} {picture_clause} {usage_clause}.".strip())
...
if __name__ == "__main__":
    ...
    dataset_id = sys.argv[2]  # "your_dataset_id"      # Replace with your BigQuery dataset ID
    table_id = sys.argv[3]

    print(project_id, dataset_id, table_id)
    copybook_output = bigquery_to_cobol(project_id, dataset_id, table_id)
